...
VERSION 4.0- One Step Closer Mode, *TEST MODE*
		AI construction completed, using minimax algorithm
		LEVEL- Easy(CHEAT)-1234/ HARD-Against AI(defualt 1)
*/
...
void playComputerGameWithoutChoice(int x, int y, int player, char tic) {
	...
	while(!checkWin() && moves!=9) {
		...
		if(player==1) {
			...
			delay(WAIT); cout<< "."; delay(WAIT); cout<< "."; delay(WAIT); cout<<  "\b\b\b   \b\b\b";
			delay(WAIT); cout<< "."; delay(WAIT); cout<< "."; delay(WAIT); cout<< ".";
			delay(800);
...
int negamax(char player1, char player2) {
	...
	if (best_move_score == -9999 || best_move_score == 0)
		...
		return best_move_score - 1; 
		//As the game goes longer, and the recursion goes deeper, 
		//the moves near the end are less favorable than in the beginning.
...
int main() {
	...
	do {
		...
		//If you are reading this, well you can decode how to win against the computer.
		//This is just a cheat lol
		if(choice==1234) {
		...
		//AI mode, choice 1 or default any other choice
		else{
